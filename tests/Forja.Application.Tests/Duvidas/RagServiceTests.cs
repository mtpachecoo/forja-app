...
public class RagServiceTests
{
    ...
    public async Task ResponderDuvidaAsync_MontaPromptComTextoDosChunksRecuperados()
    {
        ...
        _geradorDeRespostaChat
            .Setup(c => c.GerarRespostaAsync(It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .Callback<string, string?, CancellationToken>((prompt, _, _) => promptCapturado = prompt)
            .ReturnsAsync("resposta grounded");

        var resultado = await _service.ResponderDuvidaAsync(questaoId, "o que significa isso?");

        promptCapturado.Should().NotBeNull();
        promptCapturado.Should().Contain(chunks[0].Texto);
        promptCapturado.Should().Contain(chunks[1].Texto);
        promptCapturado.Should().Contain(questao.Enunciado);
    ...
    public async Task ResponderDuvidaAsync_NenhumChunkRecuperado_NaoChamaOLlmERetornaMensagemPadrao()
    {
        ...
        _geradorDeRespostaChat.Verify(
            c => c.GerarRespostaAsync(It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
